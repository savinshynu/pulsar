...
def _test_generator(script):
    ...
    def test(self):
        out, err = lint.py_run("%s -E --extension-pkg-whitelist=numpy,astropy,ephem,lsl --init-hook='import sys; sys.path=[%s]; sys.path.insert(0, \"%s\")'" % (script, ",".join(['"%s"' % p for p in sys.path]), os.path.dirname(MODULE_BUILD)), return_std=True)
